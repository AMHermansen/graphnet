...
class RelativeAttention(nn.Module):
    ...
    def forward(
        ...
        B, N, C = q.shape
        q = self.proj_q(q).reshape(B, N, self.num_heads, -1).permute(0, 2, 1, 3)
        k = self.proj_k(k).reshape(B, N, self.num_heads, -1).permute(0, 2, 1, 3)
        v = self.proj_v(v).reshape(B, N, self.num_heads, -1).permute(0, 2, 1, 3)
        q *= self.scale
...
class RelAttnTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        if self.gamma1 is None:
            ...
            x = x + self.gamma1 * self.attn(xn, kv, kv, rel_pos_bias, key_padding_mask)
            x = x + self.gamma2 * self.mlp(self.norm2(x))
        return x
...
class Extractor(nn.Module):
    ...
    def __init__(self, dim_base: int = 128, dim: int = 384):
        super().__init__()
        self.emb = SinusoidalPosEmb(dim=dim_base)
        self.proj = nn.Sequential(
            nn.Linear(
                (n_feat := (len(self._pos_idx) + len(self._t_idx) + len(self._remaining_idx))) * dim_base,
                n_feat * dim_base
            ),
    ...
    def forward(self, x: torch.Tensor, max_length: Optional[int] = None):
...
class LocalBlock(nn.Module):
    ...
    def forward(self, x, nbs, key_padding_mask: Optional[torch.Tensor] = None, rel_pos_bias: Optional[torch.Tensor] = None):
        B, max_length, C = x.shape
        mask = key_padding_mask if key_padding_mask is not None else torch.ones(B, max_length, dtype=torch.bool, device=x.device)
...
class DeepIceModelNoExtractor(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.sandwich = nn.ModuleList(
            [RelAttnTransformerBlock(dim=dim, num_heads=dim // head_size) for i in range(depth_rel)]
        )
    ...
    def forward(self, x, x_orig, mask):
        Lmax = mask.sum(-1).max()
...
if __name__ == "__main__":
    ...
    model = DeepIceModelNoExtractor()
    x = torch.arange(5*7).view(5, 7)
    batch = torch.tensor([0, 0, 0, 1, 1,])
    y = model(*torch_geometric.utils.to_dense_batch(x, batch))
    print(y.shape)
    print(f"Size of model: {sum(p.numel() for p in model.parameters())}")
