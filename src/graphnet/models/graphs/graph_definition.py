...
altering code in graphnet. These modules define what the GNNs sees as
input and can be passed to dataloaders during training and deployment.
"""
...
class GraphDefinition(Model):
    ...
    def __init__(
        ...
        sort_by: str = None,
        add_sensor_id: Optional[bool] = False,
    ):
        ...
        Args:
            ...
            add_sensor_id: If True, the sensor id will be added according to the used
                NodeDefinition. Defaults to False.
        """
        ...
        self._add_sensor_id = add_sensor_id
        ...
        # Set data type
        self.to(dtype or torch.float32)
    ...
    def forward(  # type: ignore
        ...
        if self._add_sensor_id:
            input_features = torch.cat(
                [input_features, torch.from_numpy(sensor_ids).reshape(-1, 1)],
                dim=1,
            )  # noqa

        # Create graph & get new node feature names
        graph, node_feature_names = self._node_definition(
            input_features, self._add_sensor_id
        )
        if not self._add_sensor_id:
            del graph["sensor_id"]
    ...
    def _get_sensor_ids(
        ...
        pos = input_features[:, :3].tolist()
        pos_tuple = [tuple(p) for p in pos]
        return (
            self._detector.geometry_table.loc[
                pos_tuple, self._detector.sensor_index_name
            ]
            .to_numpy()
            .astype(int)
        )
    ...
    def _geometry_table_lookup(
        ...
        """Convert xyz in `input_features` into an array of sensor ids."""
        lookup_columns = [
