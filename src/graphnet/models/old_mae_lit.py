...
class MAELit(Model):
    def __init__(
        ...
        self._mask_type_probability = masking_type_probabilities or [0.1, 0.1, 0.8]
        ...
        self.masking_intervals = torch.cumsum(
            torch.tensor(self.masking_proportions),
            dim=0
        ).numpy().tolist()
    ...
    def validation_step(self, data: Data, batch_idx: int):
        ae_loss, cls_loss, x_pred, x_true, mask, mask_type = self._compute_losses(data)
        self.log(
            'val_ae', ae_loss,
            batch_size=self._get_batch_size([data]),
            prog_bar=True,
        ...
        self.log(
            "mean_guess",
            self.ae_loss(torch.mean(x_true, dim=0).expand(x_true.shape), x_true),
            batch_size=self._get_batch_size([data]),
            prog_bar=True,
            on_epoch=True,
            on_step=False,
    ...
    def _shared_step(self, data: Data):
        ...
        mask_type = self._create_masktype(x_orig.shape[:-1], self.masking_intervals, x_orig.device)
    ...
    @staticmethod
    def _create_masktype(shape: Tuple[int], cum_mask_probabilities: List[float], device):
        ...
        for index, (p_low, p_high) in enumerate(
            zip(
                cum_mask_probabilities[:-1],
                cum_mask_probabilities[1:]
            )
        ):
            mask_type[
                (mask_type >= p_low) & (mask_type < p_high)
            ] = index + 1
        return mask_type

    def _apply_masking(self, x, mask_type):
        # mask_type: 0 - no mask, 1 - masked but original, 2 - masked and random, 3 - masked and mask_token
        x[mask_type == 2] = torch.normal(mean=torch.zeros_like(x[mask_type == 2]), std=0.2).to(x.device)
        mask_token = self.mask_token.expand(x[mask_type == 3].shape[0], 7)
        x[mask_type == 3] = mask_token

    def _compute_losses(self, data: Data):
        ...
        cls_loss = self.cls_loss(x_cls, torch.stack([data.xq_mean, data.yq_mean, data.zq_mean, data.time_duration], dim=-1))  # Make a better task for the cls token. ... Maybe predict total time span, total charge, other derived values.
