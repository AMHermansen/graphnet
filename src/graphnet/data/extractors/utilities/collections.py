...
def serialise(obj: Union[Dict, Any]) -> Union[Dict, Any]:
    ...
    It is typically not possible to save nested collections (lists,
    dicts) to file. Therefore, if `obj` is, e.g., a list of lists, we
    need to serialise each element in the outer list to JSON (i.e.,
    convert it to a JSON- formatted string) in order to be able to save
    `obj` to file. It will then be possible to de-serialise
    corresponding elements when reading them from file.
    """
    if isinstance(obj, dict):
